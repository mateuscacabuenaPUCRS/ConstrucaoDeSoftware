...
export class TenantController {
    ...
    async getAllTenants(): Promise<TenantDTO[]> {
        try {
            ...
            throw new HttpException('Failed to get tenants', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async getTenantById(@Param("id") id: number): Promise<TenantDTO> {
        try {
            ...
            throw new HttpException('Failed to get tenant', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }

    @Post()
    async createTenant(@Body() createTenantDTO: CreateTenantDTO): Promise<TenantDTO> {
        try {
    ...
    @Put(":id")
    async updateTenant(@Param("id") id: number, @Body() createTenantDTO: CreateTenantDTO): Promise<TenantDTO> {
        try {
            const tenant = await this.tenantService.getTenantById(id);
            if (!tenant) {
                throw new HttpException('Tenant not found', HttpStatus.NOT_FOUND);
            ...
            throw new HttpException('Failed to update tenant', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    async deleteTenant(@Param("id") id: number): Promise<TenantDTO> {
        try {
            ...
            throw new HttpException('Failed to delete tenant', HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
}